use students;
create table employee(empno int not null, ename varchar(20), job varchar(20), mgr int, hiredate date, salary float, comm float, deptno int);
insert into employee values(8369, 'SMITH', 'CLERK', 8902, '1990-12-18', 800.00, null, 20),
                     (8499, 'ANYA', 'SALESMAN', 8698, '1991-02-20', 1600.00, 300.00, 30),
                     (8521, 'SETH', 'SALESMAN', 8698, '1991-02-22', 1250.00, 500.00, 30),
                     (8566, 'MAHADEVAN', 'MANAGER', 8839, '1991-04-02', 2985.00, null, 20),
                     (8654, 'MOMIN', 'SALESMAN', 8698, '1991-09-28', 1250.00, 1400.00, 30),
                     (8698, 'BINA', 'MANAGER', 8839, '1991-05-01', 2850.00, null, 30),
                     (8882, 'SHIVANSH', 'MANAGER', 8839, '1991-06-09', 2450.00, null, 10),
                     (8888, 'SCOTT', 'ANALYST', 8566, '1992-12-09', 3000.00, null, 20),
                     (8839, 'AMIR', 'PRESIDENT', null, '1991-11-18', 5000.00, null, 10),
                     (8844, 'KULDEEP', 'SALESMAN', 8698, '1991-09-08', 1500.00, 0.00, 30),
                     (8886, 'ANOOP', 'CLERK', 8888, '1993-01-12', 1100.00, null, 20),
                     (8900, 'JATIN', 'CLERK', 8698, '1991-12-03', 950.00, null, 30),
                     (8902, 'FAKIR', 'ANALYST', 8566, '1991-12-03', 3000.00, null, 20),
                     (8934, 'MITA', 'CLERK', 8882, '1992-01-23', 1300.00, null, 10);
SELECT * FROM EMPLOYEE;	
select ename, salary from employee where salary >= 2200;	
update employee set column comm as commission int;

select count(comm) from employee where comm != ;

select ename, salary from employee where salary not between 2500 and 4000;
select ename, job, salary from employee where job<>'MANAGER';
select ename from employee where ename like '_a%';
select ename from employee where ename like '%t'; 	
select ename from employee where ename like 'a_y%';
		